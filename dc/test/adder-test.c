 ...
 *  Last changed:   $Date: 2010/11/15 17:34:11 $
...
char* make_updatable_input(test_t* test) {
    ...
    if (test->ra == NULL) 
        return "cannot initialize list head";
...
char* do_from_scratch_eval(test_t* test) {
    ...
    // makes adder...
    test->adder = adder_new(test->ra, &(test->sum));
...
char* do_updates(test_t* test) {
    ...
    if (rnd_updates==0)
    {
        for (; *head != NULL; head = &(*head)->next) {
            ...
            {
                int sum=0;
                rnode_t** head_check=test->ra;
                for (; *head_check != NULL; head_check = &(*head_check)->next)
                    sum=sum+(*head_check)->val;
                if (sum==test->sum)
                    return NULL;
                else
