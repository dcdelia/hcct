...
void del_test(test_t* test) {
    ...
    /*splitter_delete(test->splitter);

    rlist_remove_all(test->ra);
    rlist_remove_all(test->rout1);
    rlist_remove_all(test->rout2);
...
char* make_updatable_input(test_t* test) {
    ...
    test->rout1 = (rnode_t**)malloc(sizeof(rnode_t*));
    test->rout2 = (rnode_t**)malloc(sizeof(rnode_t*));

    if (test->ra == NULL || test->rout1 == NULL || test->rout2 == NULL) 
...
*/
    ...
    *test->ra = rlist_new_rand (test->input_size, INT_MAX);
    ...
    // output lists are initially empty
    *test->rout1 = NULL;
    *test->rout2 = NULL;
...
char* do_from_scratch_eval(test_t* test) {
    ...
    // makes halver...
    test->splitter = splitter_new(test->ra, test->rout1, test->rout2, &(test->pivot));
    if (test->splitter == NULL) return "can't build splitter";
