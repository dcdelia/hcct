 ...
 *  Last changed:   $Date: 2010/11/15 09:54:43 $
...
int main() {
    ...
    #if MAKE_DUMP_FILE
    rm_make_dump_file("logs/quicksorter-start.dump");
    #endif

    srand(131);

    *in  = NULL; //rlist_new_rand(1, MAX_VAL);
    *out = NULL;
    ...
    puts("--making quicksorter");
    quicksorter_t* h = quicksorter_new(in, out);
    if (h == NULL) {
        printf("error: cannot initialize quicksorter\n");
        exit(0);
    }
    dump(in, out, &count);

    puts("\n--adding one item to in");
    ...
    puts("\n--incrementing by 5 first item of in");
    (*in)->val += 5;
    dump(in, out, &count);
    ...
    *in = last->next;
    ...
    puts("\n--disposing of quicksorter");
    quicksorter_delete(h);
    dump(in, out, &count);
    ...
    #if MAKE_DUMP_FILE
    rm_make_dump_file("logs/quicksorter-end.dump");
    #endif
