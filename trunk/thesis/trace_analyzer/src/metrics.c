 ...
 *  Last changed:   $Date: 2011/03/17 21:34:53 $
...
void metrics_write_output(logfile_t* f, driver_t* driver,
    ...
    logfile_add_to_row   (f, "%-12.4f ", ((double)stream_hcct_weight_sum / 
                                         (double)N) * 100);    
    ...
    UINT32 hot_edges_cct, hot_edges_hcct;
    
    hot_edges_cct = larger_than_hottest(cct_get_root(),tau,H2);
    hot_edges_hcct = larger_than_hottest((cct_node_t *)hcct_root,tau,H2);
    
    //~ logfile_add_to_header(f, "%-10s ", "Hot edges");
    //~ logfile_add_to_row(f, "%-10lu", hot_edges_hcct);
    ...
    logfile_add_to_header(f, "%-9s ", "Hot cov.");
    logfile_add_to_row   (f, "%-9.4f ", 
            (((double)hot_edges_hcct)/((double)hot_edges_cct))*100);


    // tau 2x
    hot_edges_cct = larger_than_hottest(cct_get_root(),2*tau,H2);
    hot_edges_hcct = larger_than_hottest((cct_node_t *)hcct_root,2*tau,H2);            
    logfile_add_to_header(f, "%-9s ", "HC tau2x");
    logfile_add_to_row   (f, "%-9.4f ", 
            (((double)hot_edges_hcct)/((double)hot_edges_cct))*100);            
            
    // tau 4x
    hot_edges_cct = larger_than_hottest(cct_get_root(),4*tau,H2);
    hot_edges_hcct = larger_than_hottest((cct_node_t *)hcct_root,4*tau,H2);            
    logfile_add_to_header(f, "%-9s ", "HC tau4x");
    logfile_add_to_row   (f, "%-9.4f ", 
            (((double)hot_edges_hcct)/((double)hot_edges_cct))*100);            
            
    // tau 6x
    hot_edges_cct = larger_than_hottest(cct_get_root(),6*tau,H2);
    hot_edges_hcct = larger_than_hottest((cct_node_t *)hcct_root,6*tau,H2);            
    logfile_add_to_header(f, "%-9s ", "HC tau6x");
    logfile_add_to_row   (f, "%-9.4f ", 
            (((double)hot_edges_hcct)/((double)hot_edges_cct))*100);            
    
    // tau 8x
    hot_edges_cct = larger_than_hottest(cct_get_root(),8*tau,H2);
    hot_edges_hcct = larger_than_hottest((cct_node_t *)hcct_root,8*tau,H2);            
    logfile_add_to_header(f, "%-9s ", "HC tau8x");
    logfile_add_to_row   (f, "%-9.4f ", 
            (((double)hot_edges_hcct)/((double)hot_edges_cct))*100);            
    
    // tau 10x
    hot_edges_cct = larger_than_hottest(cct_get_root(),10*tau,H2);
    hot_edges_hcct = larger_than_hottest((cct_node_t *)hcct_root,10*tau,H2);            
    logfile_add_to_header(f, "%-10s ", "HC tau10x");
    logfile_add_to_row   (f, "%-10.4f ", 
            (((double)hot_edges_hcct)/((double)hot_edges_cct))*100);            
    
    logfile_add_to_header(f, "%-13s ", "min tau");
    logfile_add_to_row   (f, "%-13.9f ", (double)cct_threshold/H1);    
