...
void printTree(hcct_node_t* node, int level) {
	...
	char *file, *image;
	//~ file=printFileName(node->routine_sym->file); // file name only
	file=node->routine_sym->file; // file with full path
...
hcct_tree_t* createTree(FILE* logfile) {    
    ...
    hcct_tree_t *tree=calloc(1, sizeof(hcct_tree_t)); // zeroes are useful
...
int main(int argc, char* argv[]) {
	...
	} else printf("No value specified for phi, using default instead\n");
    ...
    if (phi==0 && (tree->tool==CCT_FULL || tree->tool == CCT_BURST || tree->tool==CCT_TIME ) ) {
		phi=1.0/(EPSILON/10);		
    } else {
		// LSS_FULL, LSS_BURST, LSS_TIME
	...
	if (tree->tool==CCT_BURST || tree->tool==LSS_BURST)
		min_counter=(UINT32)(phi*tree->sampled_enter_events);		
	else if (tree->tool==CCT_TIME || tree->tool==LSS_TIME)
		min_counter=(UINT32)(phi*tree->thread_tics);
	else // CCT_FULL, LSS_FULL
		min_counter=(UINT32)(phi*tree->exhaustive_enter_events);		
